...
class MCPCore:
    def __init__(self, conn):
        ...
        conn.output_pane.register_line_filter('mcp', self.output_filter)
    ...
    def output_filter(self, output_pane, data):
        ...
        while p:
            ...
            if matches > 0:
                return_val += line # we did, and removed it.  Add the line
                line, p, rest = rest.partition('\n')
                continue

            line, matches = utility.OOB_PREFIX.subn('', line) # did we have the oob prefix?
            if matches == 0:
                return_val += line # we did not, so add the line and bail
                line, p, rest = rest.partition('\n')
                continue
